...
import com.my.gateway.config.KafkaProperties;
...
class GatewayV4KafkaResourceIT {
    ...
    void setup() {
        ...
        GatewayV4KafkaResource kafkaResource = new GatewayV4KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gateway-v-4-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gateway-v-4-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
